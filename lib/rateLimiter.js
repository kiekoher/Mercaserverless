...
export async function checkRateLimit(req, { limit = 10, userId = null } = {}) {
  ...
  // --- Fallback Strategy ---
  // If Redis is not available or not connected, decide based on environment
  if (process.env.NODE_ENV === 'production') {
    logger.error('Redis is not available for rate limiting in PRODUCTION. Request blocked.');
    return false; // Fail closed in production
  }

  // In non-production environments, use the in-memory store as a fallback
  logger.warn({ identifier }, 'Redis not available. Using in-memory rate limiting for development.');
  const now = Date.now();
